...
class DataTrainingArguments:
    ...
    top_k: Optional[int] = field(
        ...
        metadata={
            "help": (
                " to use for evaluation. This argument will be passed to ``model.generate``, "
                "which is used during ``evaluate`` and ``predict``."
            )
        },
    )
...
def main():
    ...
    if training_args.do_eval:
        ...
        num_beams=num_beams, top_k=data_args.top_k, temperature=data_args.temperature, top_p=data_args.top_p, do_sample=data_args.do_sample)
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            ...
            output_prediction_file = data_args.output_file_name

            with jsonlines.open(output_prediction_file, "w") as writer:
                for test_dict, title in zip(test_data, predictions):
                    writer.write({
                        "title": title,
                        "id": test_dict["id"]
                    })
